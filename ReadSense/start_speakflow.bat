...
start "SpeakFlow Backend" cmd /k "cd /d %ROOT%backend && python -m uvicorn main:app --reload"

:: Start the Next.js Frontend in a new command prompt window
start "SpeakFlow Frontend" cmd /k "cd /d %ROOT%frontend && npm run dev"

echo Both servers are starting up!
echo The Backend will be available at http://127.0.0.1:8000
echo The Frontend will be available at http://localhost:3000
echo   - Teacher Hub:            http://localhost:3000/teacher
echo   - Child Mode (Story Play): http://localhost:3000/read
