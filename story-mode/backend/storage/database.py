...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        try:
            await db.execute('ALTER TABLE session_words ADD COLUMN transcribed_word TEXT')
        except sqlite3.OperationalError:
...
async def save_checkpoint_phase1(session_id: str, checkpoint_id: str, transcript: str, stt_source: str, words: list):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Save words
        word_records = [(session_id, checkpoint_id, w['word'], w['correct'], w.get('phoneme_mismatch'), w.get('transcribed_word')) for w in words]
        await db.executemany('''
            INSERT INTO session_words (session_id, checkpoint_id, word, correct, phoneme_mismatch, transcribed_word)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', word_records)
        await db.commit()
