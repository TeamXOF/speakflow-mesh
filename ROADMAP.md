...
## 📌 STATUS AUDIT — 2026-09-06 (code-level, verified by running — not checkbox-trust)

**Important repo-state correction:** this file's "Current state of the repo" note says there is no codebase. The actual repo contains the complete July **Gen-1 app** (5-agent Gemma engine dashboard, preserved per the product owner's explicit instruction) **plus** the full v2 layer specified by this roadmap. Frontend teacher pages live under `app/(app)/` (route group with the sidebar shell); kid-facing Story Mode lives at `app/read/` **outside** the group = fullscreen, no chrome.

| Phase | True status | Notes |
|---|---|---|
| FE 0, 0.5 | ✅ done | Design dissected; shell pre-existed (Gen-1) rather than created from scratch |
| FE 1–5, 5.5 | ✅ done with deviations | Real API from day one (no mock stages). Route = `/read` + `/read/[sessionId]` (sessionId-based per backend contract). Fullscreen kid shell ✅. Word chips: green/amber/red mapping ✅. Next Checkpoint gated on API Phase 2 ✅. Per-checkpoint rewards interstitial ✅. Star badge ✅. Mascot is emoji-based, not a designed character |
| FE 6 / T1 | ✅ done | Lives at `/teacher` (NOT `/dashboard` — that route intentionally preserves the Gen-1 engine console). Real data, 4 stat cards, 7-day chart, struggling areas |
| FE 6.5 / T0 | ✅ done | Gen-1 roster + row-click → `/students/[id]` |
| FE 7 / T2 | ✅ done | `/students/[id]`: 4 metrics, trend chart, struggling words + sound patterns, story progress, parent card |
| FE 8 / T3 | ✅ done | `/sessions/[id]` dual-mode (story + legacy). Per-word chips, engagement badge, source badges, recommended practice |
| FE 9 / T4 | ✅ done | Checkpoint overview table on `/teacher` (per-checkpoint avg accuracy + passed counts) |
| FE 10 | ✅ done | Full API client + session context (WS + polling fallback), multipart Blob upload |
| FE 11 | ✅ done | Settings (mode, Groq/Gemini indicators, privacy note, Sync Now) + **`/pipeline` Pipeline Monitor** (4 stage cards, real latencies, live log). Old `/agents` fake page redirects to it |
| FE 12 | ✅ done | Real data everywhere; per-sound heatmap on `/teacher`; merged sessions list with **real durations** (Story) and honest "—" (legacy sessions track no duration); `stt_source`/`feedback_source` badges |
| FE 13 | ✅ done | Language picker, Nastaliq via **self-hosted** `next/font/local` (fonts.googleapis.com is unreachable on this machine), RTL only on text blocks |
| FE 14 | ✅ done | Parent WhatsApp preview card on student detail (inline, not a separate component file) |
| FE 15 / X | ✅ executed | tsc clean, production build passes (16 routes), all sidebar routes 200, backend-online/offline indicator, warnings→toasts. Accepted deviations: Gen-1 artifacts (`MOCK_SESSION_DATA`, `runPipeline`, `gemma` copy, one hardcoded base URL const) remain intentionally because the Gen-1 engine console is preserved by product decision |
| BE A–K | ✅ done | `services/` layout (flat) instead of `app/services/*` subpackages. **Groq STT live-verified** with a real key (word timestamps ✓). faster-whisper INT8 preferred with openai-whisper fallback. Reference dictionary + manual overrides + LPC formants (F1/F2) + acoustic blending implemented; dictionary vectors are **clearly-labeled placeholders** until native-speaker recordings are run through `scripts/calibrate.py --ref`. Error envelope implemented for `/api/v1`. `--local-host` flag added. `GEMINI_API_KEY_DEMO`/`ENV` demo-key switching supported (set `ENV=demo` on demo day) |
| Phase X latency item | ⚠️ known limit | Online STT now ~<1s via Groq; local CPU path (Whisper tiny + librosa) remains multi-second and is logged per-stage. `total_phase1 < 800ms` is achievable only on the Groq path with short clips — measure on demo hardware |
| K.2 calibration | 🛠 tooling delivered | `scripts/calibrate.py` builds the confusion matrix from `test_audio/` recordings (`<story_id>__<checkpoint_id>.ok1.wav` / `.bad1.wav`) and prints per-word stability + reference-vector extraction (`--word X --ref`). **Must be run with real child recordings before the pitch** — the dictionary placeholders are not calibrated data |

### 📌 STATUS AUDIT ADDENDUM — 2026-09-06 second pass (code-level, verified by running)

Continuation session. Full codebase re-read; both test suites re-run on a fresh DB; backend booted and CORS verified live. Changes:

| Area | Change | Verified how |
|---|---|---|
| Tests | `test_v2_pipeline.py` now logs in (teacher token) for session-create + dashboard + classroom calls — auth enforcement had silently broken it. `_auth_test.py` made idempotent (unique usernames per run) | Both suites: **PASSED** on fresh DB |
| K.2 tooling | `calibrate.py` rewritten where broken: `--ref` now extracts features over the word's exact STT timespan using the same runtime pipeline (`extract_word_acoustics`) — previously whole-file stats, which would have produced systematically offset vectors; honors `--language` (uses `text_ur` for ur); removed dead DB import; fixed naming-doc mismatch (double underscore); new `--write` merges vectors into `manual_overrides.json` with backup; **fixed a silent glob failure on paths containing `[`/`]` (this project's path)** by switching to pathlib | Smoke-tested end-to-end with a real mp3 take: STT → alignment → matrix prints; `--ref` error path clean |
| Kid flow | Phase-2 polling timeout no longer deadlocks a **passed** checkpoint ("Next Checkpoint" appears once feedback errors out); WS socket closed before re-adopt (leak); unlinked student accounts get a clear alert instead of a silent no-op; KidNav hides the dead "My Progress" link when `student_id` is null; mock path no longer shows "STT fell back to the offline model" to kids | Code review + tsc + build |
| LAN demo | Frontend API host now derives from `window.location.hostname` (all 4 former hardcoded `127.0.0.1` spots centralized in `lib/api.ts`); backend CORS accepts private-range origins (any port) via `allow_origin_regex` — public origins still denied | Live boot: LAN origin allowed, public origin denied, `/health` + `/api/v1/health` OK |
| Mascot | Emoji placeholders replaced by a designed SVG star mascot (`frontend/components/read/Mascot.tsx`, moods: happy/encourage/thinking/celebrate) across Phase-1 result, Phase-2 bubble, rewards, celebration | tsc clean, build 19 routes |
| Small fixes | teacher.py `logging` name-clobber; stt.py always-0.9 expression; dead `_silent_placeholder`/`prev_done` removed; sessions-list date parsing (Gen-1 `YYYY-MM-DD HH:MM:SS` → NaN in Safari/Firefox) normalized; Topbar shows real user + working Sign out + role-aware Settings; practice page invalid Tailwind classes | tsc + build + suites |
| Still open | K.2 needs **real child recordings** (human task); latency measurement on demo hardware; one real LAN run on tablets | — |
